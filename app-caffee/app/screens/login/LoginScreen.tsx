  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        "http://10.18.6.196:8080/api/login",
      ...
      if (token) {
        ...
        console.log("OTP giả định:", otp);

        Alert.alert("Mã OTP đã được gửi", `OTP của bạn là: ${otp}`);

        // Chuyển sang màn Otp
        navigation.navigate("Otp", { username, otp });
        setErrorMessage("");
      } else {
...
const styles = StyleSheet.create({
  ...
  signUpText: { flexDirection: 'row', justifyContent: 'center', textAlign: "center", marginBottom: 20 },
  loginTextContent: { color: '#A58446', fontWeight: 'bold', textAlign: "center" },
  changePasswordText: { justifyContent: 'center', alignItems: 'center', marginTop: 10 },
  changePasswordLinkText: { color: '#A58446', fontWeight: 'bold' },
